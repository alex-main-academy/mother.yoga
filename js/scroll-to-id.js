function anchorSetLoadPage() {
  ...
  if (window.location.hash.substr(1) !== "") {
    ...
    const menuActive = document.querySelectorAll(
      ".menu > .menu__nav > .ul > .menu__item"
    );
    ...
    if (document.getElementById(anchor) == null) {
      ...
      document
        .querySelector(
          ".menu > .menu__nav > .ul > .menu__item[data-id=" + anchor + "]"
        )
...
const anixBox = document.querySelectorAll(".anim-box");

if (window.innerWidth < 768) {
  anixBox.forEach((elem) => {
    elem.classList.remove("anim-box");
    elem.classList.remove("anim-scroll");
  });
}
